...
startup() {
  ...
  helm upgrade --install $INFRA_NAME test-294-infrastructure \
     --values test-294-infrastructure/values.yaml \
     --values test-294-infrastructure/values-dev.yaml
  if ! kubectl rollout status --namespace argocd deployment/argocd-server --timeout=30s; then
    exit $?
  fi
  argocd repo add https://github.com/jaebchoi/test-294 --server localhost:30080 --plaintext --insecure-skip-server-verification
}
...
deploy() {
  ...
  if is_app_running; then
    echo "test-294 is deployed"
  else
    branch=$(git rev-parse --abbrev-ref HEAD)
    echo "Deploying test-294 from branch '$branch'..."
    argocd app create $APP_NAME \
      --server localhost:30080 --plaintext \
      --dest-namespace test-294 \
      --dest-server https://kubernetes.default.svc \
      --repo https://github.com/jaebchoi/test-294 \
      --path test-294-deploy/src/main/resources \
      --revision $branch \
